...
int acc_example_detector_distance();
...
static void cleanup(distance_detector_resources_t *resources)
{
	...
	free(resources->detector_cal_result_static);
...
static bool initialize_detector_resources(distance_detector_resources_t *resources)
{
	...
	resources->detector_cal_result_static = malloc(resources->detector_cal_result_static_size);
	...
	for (uint16_t i = 0; !status && (i <= calibration_retries); i++)
	{
		...
		do
		{
			...
			if (status && !cal_complete)
			{
				...
				status = Radar_WaitForInterrupt(SENSOR_TIMEOUT_MS);
			}
                                 ...
                                 acc_detector_distance_result_t *result)
{
	...
	do
	{
		...
		if (!Radar_WaitForInterrupt(SENSOR_TIMEOUT_MS))
		{
